...
RPC.register('serenity-phone:debt', function(source)
    local src = source
...
function buildInfo(house)
    ...
    for k,v in pairs(house) do
        ...
        local street = exports['serenity-housing']:HouseStreet(v.hid)
...
RPC.register('serenity-phone:debtPrice', function(source, id)
    local src = source
    local hId = id.param
    return getPrice(hId)
end)
...
RPC.register('serenity-phone:car_payment', function(source, pl)
    local src = source
...
RPC.register('serenity-phone:h_payment', function(source,id,pay)
    local src = source
    local hId = id.param
...
RPC.register('serenity-phone:p_payment', function(source,id,p,did)
    local src = source
    local pId = id.param
    local dId = did.param
...
RPC.register('serenity-phone:addBill', function(source,id,a,t,c)
    local src = source
    local cid = id.param
