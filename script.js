...
$(document).ready(function () {
    ...
    $.post('http://serenity-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function (data) {
        ...
        if (data.which == 27) {
            closePhoneShell()
            $.post('http://serenity-phone/close', JSON.stringify({}));
        }
    }

    $(".phone-screen").on('click', '#wifi', function (e) {
        if (wifizone){
            $.post('https://serenity-phone/connectWifi', JSON.stringify({}));
        }
    });
    setInterval(function () {
        var d = new Date();
        ...
        if (actionButton !== undefined) {
            switch (actionButton) {
                case "home":
                    ...
                    globalyhide()
                    $.post('https://serenity-phone/refreshMySMS', JSON.stringify({status: false}));
                    ...
                    if(callStates[currentCallState] !== "isCallInProgress" && callStates[currentCallState] !== "isDialing" && activePing !== true){
                        ...
                        // closeOtherContainer()
                        $.post('http://serenity-phone/closeTwat')
                    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://serenity-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://serenity-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://serenity-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://serenity-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://serenity-phone/' + action, JSON.stringify({}));
                    break;
                case "spotify":
    ...
    window.addEventListener('message', function (event) {
        ...
        if(item.callisAnswer == true){
            ...
            // $.post('http://serenity-phone/btnAnswer', JSON.stringify({}));
        }
        ...
        switch (item.openSection) {
            ...
            case "messageRead":
                ...
                $.post('https://serenity-phone/refreshMySMS', JSON.stringify({status: true}));
            ...
            case "twatter":
                ...
                unReadTwt = 0
                $.post('http://serenity-phone/updateTwatter');
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://serenity-phone/race:completed', JSON.stringify({
...
">${moment.utc(emails[email].time).local().fromNow()}
    ...
    $.post('http://serenity-phone/emailReads', JSON.stringify({}));
}
...
function openBrowser(url) {
    ...
    $.post('http://serenity-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function purchaseDarkMarket(data) {
    $.post("https://serenity-phone/purchase-darkMarket", JSON.stringify({item:data,itemamount}))
}

function trackvehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://serenity-phone/vehtrack", JSON.stringify({vehplate}))
}

function spawnVehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://serenity-phone/vehspawn", JSON.stringify({vehplate}))
}

function sellVehicle(vehplate) {
    $.post("https://serenity-phone/sellVeh", JSON.stringify({vehplate}))
}

function gpsJobCenter(data) {    
    $.post("https://serenity-phone/jobcenter-setgps", JSON.stringify({item:data}))
}
...
$("#selfieclose").click(function () {
    $('.selfieBox').hide();
    $.post('http://serenity-phone/closeSelfi', JSON.stringify({}));
})
...
//<span class="new-badge" style="margin-left: 10px;margin-top: 40px;" data-badge-caption="">${moment(date).local().fromNow()}</span>
    ...
    element.click(function () {
        if(item.unread === 0){
            $.post('http://serenity-phone/readMessage', JSON.stringify({ id: item.id }));
        }
        $.post('http://serenity-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://serenity-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
$(document).ready(function () {
    ...
    $(".jss47").click(function () {
        ...
        openContainer("messages")
        // $.post('http://serenity-phone/messages', JSON.stringify({}));
...
$("#contact-submit-input").click(function (e) {
    ...
    complateInputJustGreen();
    setTimeout(() => {
        $.post('http://serenity-phone/newContactSubmit', JSON.stringify({
            name: clean,
...
$(document).ready(function () {
    ...
    $('.send-ping').click(function () {
        ...
        // $.post('http://serenity-phone/btnCamera', JSON.stringify({}));
        $.post('http://serenity-phone/serenity-ui:pingSend', JSON.stringify({
            number: targetId
        }));
        $('.send-value').val('');
    });
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://serenity-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate", controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://serenity-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://serenity-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://serenity-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://serenity-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://serenity-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://serenity-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    ...
    $.post('http://serenity-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://serenity-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://serenity-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://serenity-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://serenity-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://serenity-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://serenity-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://serenity-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-edit-submit").click(function (e) {
    ...
    $.post('http://serenity-phone/editContactSubmit', JSON.stringify({
        name: clean,
    ...
    setTimeout(() => {
        $.post('http://serenity-phone/refreshContacts', JSON.stringify({}));
    }, 2000);
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://serenity-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('https://serenity-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#stock-buy-form").submit(function (event) {
    event.preventDefault();
    $.post('https://serenity-phone/exchangeCrypto', JSON.stringify({
...
//     event.preventDefault();
//     $.post('http://serenity-phone/newTwatSubmit', JSON.stringify({
...
$(".submit-bg").click(function (event){
    if ($('#bg-set-body').val() == '') {
        ...
        $.post('http://serenity-phone/submitBg', JSON.stringify({
            bg: bg
        }));
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://serenity-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://serenity-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://serenity-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
...
//             setTimeout(function () {
//                 $.post('http://serenity-phone/messageRead', JSON.stringify({
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://serenity-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://serenity-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://serenity-phone/btnNotifyToggle', JSON.stringify({}));
});



$('.account-information-toggle-pager').click(function () {
    $.post('http://serenity-phone/btnPagerToggle', JSON.stringify({}));
...
function clickImage(img) {
    $('.selfieBox').click(function(){
        copyToClipboard(img);
        $.post('http://serenity-phone/closeSelfieBox', JSON.stringify({}));
    });
}
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://serenity-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://serenity-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://serenity-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://serenity-phone/removeHouseKey', JSON.stringify({
...
$('.group-manage-entries').on('click', '.group-manage-fire', function () {
    ...
    $.post('http://serenity-phone/serenity-phone:fireEmp', JSON.stringify({ id: $(this).data('id'), group: $(this).data('group'), name: $(this).data('name') }));
});

$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://serenity-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://serenity-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://serenity-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://serenity-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://serenity-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://serenity-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://serenity-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://serenity-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.jss1481, .calls-container').on('click', '.btn-contacts-call', function () {
    $.post('http://serenity-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://serenity-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    $('#confirm-modal').modal('close');
});

$('#deleteting-yp').click(function (event) {
  $.post('http://serenity-phone/deleteYP', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$("#h-check").click(function () {
    ...
    $.post('http://serenity-phone/checkHouse', JSON.stringify({}))
})

$("#edit-h").click(function () {
    // console.log("CHECK PROPERTY")
    // $.post('http://serenity-phone/editHouse', JSON.stringify({}))
    noPropty()
})

$("#sell-h").click(function () {
    // console.log("CHECK PROPERTY")
    $.post('http://serenity-phone/sellsHouse', JSON.stringify({}))
})
...
$('.h-found').on('click', '.h-found-purchase', function (event) {
    ...
    $.post('http://serenity-phone/purchaseHouse', JSON.stringify({
...
$('.sell-house-own').on('click', '.confirm-sellHouse', function (event) {
    ...
    $.post('http://serenity-phone/sellOwnHouse', JSON.stringify({
        hid: $('#h-p-id').val()
    }))
})
...
$('#submit-h-price').click(function(){
    ...
    $.post('http://serenity-phone/editHousePrice', JSON.stringify({
        hid: hid,
        price: price
    }))
    $('.housing-edit-price').css('display', 'none');
...
$("#twat-gifword").change(function() {
   ...
   $(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://serenity-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://serenity-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
if (app === "icall" || app === "rcall" || app === "incall"){
                    ...
                    if (count <= 1) {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                $.post('http://serenity-phone/btnHangup', JSON.stringify({}));
                            }, 1000)
                        }, 200)
                    }
    ...
    setTimeout(() => {
        ...
        if (Notif) {
            ...
            Notif.addClass('notification-container-fade-out')
            $.post('http://serenity-phone/pingEnded', JSON.stringify({}));
            setTimeout(() => {
                Notif.remove()
                var count = 0
                $.each($(".notification-container"), function(i, d) { count =	count + 1 })
    ...
    $(document).ready(function () {
        ...
        $(".notification-container").on('click', '#ping-accept', function () {
            $.post('http://serenity-phone/serenity-ui:pingAccept', JSON.stringify({}));
            const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
                if (Notif) {
                    var count = 0
                    $.each($(".notification-container"), function(i, d) { count =	count + 1 })
        ...
        $(".notification-container").on('click', '#ping-reject', function () {
            activePing = false
            $.post('http://serenity-phone/serenity-ui:pingReject', JSON.stringify({}));
            const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
                if (Notif) {
                    var count = 0
                    Notif.remove()
...
$(".house-entries, .access-house-entries").on('click', '.h-setgps', function (e) {
    ...
    $.post('http://serenity-phone/sethGps', JSON.stringify({hid: overid}));
})
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://serenity-phone/unlockHouses', JSON.stringify({hid: overid, status: hstatus}));
 })
 $(".house-entries").on('click', '.h-edit-s', function (e) {
    $.post('http://serenity-phone/editHousingOwn', JSON.stringify({}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    e.preventDefault();
    overid = $(this).attr("houseId")
    $.post('http://serenity-phone/houseAccess', JSON.stringify({
        hid: overid
    }));
 })
 ...
 $('.submit-h-acess').click(function(){
    ...
    $('.phone-acces-house').hide()
    $.post('http://serenity-phone/giveAccess', JSON.stringify({
        hid: hid,
        cid: cid
    }));
})
