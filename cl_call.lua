...
function answerPhoneCall()
    ...
    TriggerServerEvent("serenity-phone:callAccept", incomingCallId)
end

function endPhoneCall()
    if isRinging then
        TriggerServerEvent("serenity-phone:callDecline", incomingCallId)
        -- phoneNotification("fas fa-phone-volume", "Call", "Not answered", 3000)
    elseif isDialing then
        TriggerServerEvent("serenity-phone:callDecline", incomingCallId)
        -- phoneNotification("fas fa-phone-volume", "Call", "Not answered", 3000)
    elseif activeCallId then
        TriggerServerEvent("serenity-phone:callEnd", activeCallId)
        SendNUIMessage({
...
RegisterNetEvent("serenity-inventory:itemCheck")
AddEventHandler("serenity-inventory:itemCheck", function(itemId, hasItem)
...
RegisterNUICallback("callContact", function(data, cb)
    ...
    local caller_number = RPC.execute("serenity-phone:getNumber")
    local target_number = tonumber(data.number)

    if not caller_number or not target_number then return end

    TriggerServerEvent("serenity-phone:callStart", caller_number, target_number)
    closeGui()
...
RegisterNetEvent('serenity-phone:endCall')
AddEventHandler('serenity-phone:endCall', function()
    endPhoneCall()
    closeGui()
    SendNUIMessage({
        openSection = 'callnotifyEnd'
...
RegisterCommand("+hangup", function()
    TriggerEvent("serenity-phone:endCall")
  end, false)
  ...
  Citizen.CreateThread(function()
    exports["serenity-keymapping"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
  
    exports["serenity-keymapping"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)