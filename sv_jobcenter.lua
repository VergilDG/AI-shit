...
RPC.register("serenity-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("serenity-phone:getMyGroup", function(source)
    local src = source
    local idle = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("serenity-phone:c_group", function(source)
    local src = source
...
RPC.register("serenity-phone:j_group", function(source, id)
    local src = source
    local user = exports["serenity-core"]:getModule("Player"):GetUser(src)
...
RPC.register("serenity-phone:get_groupsMem", function(source, id)
    local src = source
    local user = exports["serenity-core"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
...
RPC.register('serenity-phone:g_ready', function(source,stat,id)
    local src = source
...
RPC.register('serenity-phone:leave_group', function(source,id)
    local src = source
    local gId = id.param
...
--         end
--       end
--     --   TriggerLatentClientEvent('mhserenity-housing:collectHouses', -1, 30000, Houses)
--     end)
-- end)