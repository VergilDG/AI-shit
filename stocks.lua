...
RegisterNUICallback('buying-darkMarket', function()
  SendNUIMessage({
    openSection = "buydarkMarket", materials = exports['serenity-phone']:getBuying(), blueprintid = RPC.execute("serenity-phone:blueprintid")
  })
end)
...
RegisterNUICallback('stocksTradeToPlayer', function(data, cb)
  ...
  if data.identifier == "Syndite" then
      status = RPC.execute("serenity-phone:buyCrypto",1,data.amount)
    if status then
  ...
  elseif data.identifier == "TGB Coin" then
    status = RPC.execute("serenity-phone:buyCrypto",2,data.amount)
    if status then
  ...
  elseif data.identifier == "DVD Prime" then
    status = RPC.execute("serenity-phone:buyCrypto",3,data.amount)
    if status then
...
RegisterNUICallback('exchangeCrypto', function(e)
  ...
  local characterId = tonumber(exports["serenity_manager"]:isPed("cid"))
  ...
  if playerid == characterId then
    RPC.execute("serenity-phone:exchangeCrypto",popo,amount)
  else
    RPC.execute("serenity-phone:transferCrypto",popo,amount,playerid)
  end
...
function sendStocksToPhone(isRefresh)
  ...
  if isRefresh then
    ...
    serverstockvalues = RPC.execute("serenity-phone:incomingCrypto")
  else
    serverstockvalues = RPC.execute("serenity-phone:incomingCrypto")
  end

  Synditelol = RPC.execute("serenity-phone:GetSyndite")
  Tgblol = RPC.execute("serenity-phone:GetTgb")
  Dvdlol = RPC.execute("serenity-phone:GetDvd")
...
RegisterNUICallback('purchase-darkMarket', function(data)
    local characterId = exports["serenity_manager"]:isPed("cid")
    local colorLaptop = exports['serenity-phone']:getBuying()
    local time = math.random(60000,120000)

    if orderItem == false then
      if data.item == "vpn" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',1,20)
          if result then
            itemDM = "vpnxj"
      ...
      elseif data.item == "deliverylist" then
        ...
        elseif data.item == "choplist" then
          result = RPC.execute('serenity-phone:purchaseDarkMarket',2,20)
            if result then
              itemDM = "choplist"
      ...
      elseif data.item == "gdongle" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',2,100)
          if result then
            itemDM = "heistusb4"
      ...
      elseif data.item == "boostlaptop" then
        -- result = RPC.execute('serenity-phone:purchaseDarkMarket',1,100)
        -- if result then
      ...
      elseif data.item == "laptopred" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',1,65)
        if result then
          itemDM = "heistlaptop4"
      ...
      elseif data.item == "laptopgreen" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',1,50)
          if result then
            itemDM = "heistlaptop3"
            TriggerEvent("chatMessage", "Suspicious Individual", 8, "We've received an order for your laptop, please wait a few minutes!")
            Wait(time)
      ...
      elseif data.item == "thermite" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',2,50)
          if result then
            itemDM = "thermite"
      ...
      elseif data.item == "Skorpion" then
        result = RPC.execute('serenity-phone:purchaseDarkMarket',3,data.itemamount)
        if result then
...
RegisterNetEvent('serenity-phone:takeDMarket')
AddEventHandler('serenity-phone:takeDMarket', function()
  if orderItem then
