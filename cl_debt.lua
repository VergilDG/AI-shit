RegisterNUICallback('btnDebt', function()
    local cars, house, debt = RPC.execute('serenity-phone:debt')
    SendNUIMessage({
        openSection = "debt",
        cars = cars,
        house = housing(house),
...
RegisterNUICallback('car_paid', function(data)
    ...
    local complete = RPC.execute('serenity-phone:car_payment', plate,payment)
    if complete then
...
RegisterNUICallback('h_paid', function(data)
    local id = data.hid
    local payment = data.payment
    local complete = RPC.execute('serenity-phone:h_payment', id,payment)
    if complete then
...
RegisterNUICallback('p_paid', function(data)
    ...
    local complete = RPC.execute('serenity-phone:p_payment',id,payment,did)
...
RegisterNetEvent('serenity-phone:MakeBill', function()
    -- print("MAKING BILLS")
    local input = exports["serenity-input"]:showInput({
        {
            icon = "id-card",
            label = "CID",
            name = "cid",
    ...
    if input["cid"] ~= nil and input["amount"] ~= nil then
        ...
        local complete = RPC.execute("serenity-phone:addBill",cid,amount,type,comment)
        if complete then
...
RegisterCommand('signin', function()
    TriggerServerEvent("serenity-signin:duty", "police")
end)