...
AddEventHandler('Client:UpdateTweets', function(data)
    local handle = exports["serenity_manager"]:isPed("twitterhandle")
    local tweet = {}
...
AddEventHandler('Client:UpdateTweet', function(tweet)
    ...
    if string.find(tweet["message"],handle) then
      ...
      if pNotify() and exports['serenity-phone']:pOpen() then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('serenity-phone:twatSendNotification', handle, data, data.time)
