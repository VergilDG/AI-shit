function GroupName(groupid)
    ...
    local mypasses = exports["serenity_manager"]:isPed("passes")
    for i=1, #mypasses do
...
RegisterNUICallback('addEmployee', function(data)
    ...
    local complete = RPC.execute('serenity-phone:business_hired', business,cid,rankid)
...
RegisterNUICallback('editEmployee', function(data)
  ...
  local complete = RPC.execute('serenity-phone:business_edit', business,cid,rankid)
...
RegisterNUICallback('fireEmployee', function(data)
    ...
    local complete = RPC.execute('serenity-phone:business_fire', business,cid)
    if not complete then
...
RegisterNUICallback('payCheckEmployee', function(data)
    ...
    local complete, paid = RPC.execute('serenity-phone:business_paycheck', business,cid,amount)
    if complete then
...
RegisterNUICallback('btnTaskGroups', function()
    ...
    -- local groups,bank,gIp = RPC.execute("serenity-phone:Groups")
    SendNUIMessage({
      openSection = "groups",
      groups = groupObject
    })
  ...
  AddEventHandler("group:fullList", function(result,bank,groupid)
    local myjob = exports["serenity_manager"]:isPed("myjob")
  ...
  RegisterNUICallback('serenity-phone:fireEmp', function(data,cb)
    TriggerServerEvent('serenity-phone:fireEmp', data.id,data.group,data.name)
  end)

  RegisterNUICallback('addCashBank', function(data)
    local bid = data.bid
    local amount = data.amount
    local bank = RPC.execute("serenity-phone:bus_addBank",bid,amount)
    if bank then
