...
RPC.register('serenity-phone:business_hired', function(source,bus,p,r)
    local src = source
    local bId = bus.param
    local cid = p.param
    local rank = r.param
...
RPC.register('serenity-phone:business_edit', function(source,bus,p,r)
    local src = source
    local bId = bus.param
    local cid = p.param
    local rank = r.param
...
RPC.register('serenity-phone:business_paycheck', function(source,bus,p,a)
    local src = source
    local bId = bus.param
    local cid = p.param
...
RPC.register('serenity-phone:bus_addBank', function(source,id,a)
    local src = source
    local user = exports["serenity-core"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
    local bId = id.param
...
RPC.register('serenity-phone:business_fire', function(source,bus,p)
    local src = source
    local bId = bus.param
    local cid = p.param
    local affectedRows = MySQL.update.await([[
