...
RegisterNUICallback('messages', function(data, cb)
  loadSMS(exports['serenity_manager']:isPed('cid'))
  cb('ok')
end)

function loadSMS(cid)
  local messages, phone = RPC.execute("serenity-phone:getSMS",cid)
  local _messages = {}
...
local data = message
local app = "sms"
if exports["serenity-phone"]:phasPhone() then
  if exports['serenity-phone']:pNotify() == true then
...
if(number ~= nil and message ~= nil) then
  TriggerServerEvent('phone:sendSMS', exports['serenity_manager']:isPed('cid'), number, message)
  Citizen.Wait(1000)
  TriggerServerEvent('phone:getSMSc', exports['serenity_manager']:isPed('cid'))
else
...
if lastnumber ~= 0 then
  TriggerServerEvent('phone:sendSMS', exports['serenity_manager']:isPed('cid'), lastnumber, message)
...
TriggerServerEvent('phone:getSMS', exports['serenity_manager']:isPed('cid'))
Citizen.Wait(250)
